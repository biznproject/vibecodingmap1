...
import { Cpu, Terminal, ShieldCheck, Zap, Lock, Unlock, Copy, Check, ArrowRight } from "lucide-react";
...
export default function X402MarketplacePage() {
  ...
  const handleSimulatePayment = async () => {
    ...
    // Simulate HTTP 402 challenge step
    setTimeout(async () => {
      try {
  ...
  return (
    <div className="min-h-screen bg-black text-white selection:bg-cyan-500/30 pt-28 pb-20 px-6 max-w-6xl mx-auto">
      {/* Header with Bauhaus Accents */}
      <div className="mb-12 border-b-4 border-white/20 pb-8 flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          ...
          <p className="text-sm font-mono text-gray-400 mt-2">
            Autonomous Machine-to-Machine Knowledge Marketplace for AI Coding Agents
          </p>
        </div>
        ...
        <div className="flex items-center gap-3 bg-white/5 border border-white/10 px-4 py-3 rounded-xl">
          ...
          <div className="text-xs font-mono">
            <div className="text-gray-400">Facilitator Settlement</div>
            <div className="font-bold text-white">0.01 USDC / Request</div>
          </div>
        </div>
      </div>

      {/* Grid Layout: Catalog List vs Interactive Simulator */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        
        {/* Left Column: Available Blueprints Catalog */}
        <div className="lg:col-span-5 space-y-4">
          <h2 className="text-xs font-mono tracking-widest uppercase text-gray-400 flex items-center gap-2">
            <Cpu className="w-4 h-4 text-cyan-400" /> Verified Spec Catalog
          </h2>
          ...
          <div className="p-6 rounded-2xl bg-gradient-to-br from-indigo-950/40 to-purple-950/40 border border-indigo-500/20 mt-8">
            <div className="flex items-center gap-2 mb-2 text-xs font-mono font-bold text-indigo-300">
              <Terminal className="w-4 h-4" /> FastMCP Server Ready
            </div>
            <p className="text-xs text-gray-300 mb-3">
              Add VibeCodingMap directly to Cursor / Claude Desktop to allow your agent to auto-purchase specs:
            </p>
            <div className="p-3 bg-black/80 rounded-lg font-mono text-[11px] text-cyan-300 border border-white/10">
        ...
        <div className="lg:col-span-7 space-y-4">
          <h2 className="text-xs font-mono tracking-widest uppercase text-gray-400 flex items-center gap-2">
            <Zap className="w-4 h-4 text-bauhaus-yellow" /> Live Protocol Inspector
          </h2>

          <div className="bg-white/5 border border-white/10 rounded-2xl p-6 relative overflow-hidden backdrop-blur-md">
            
            {/* Spec Title & Price Bar */}
            <div className="flex justify-between items-center pb-4 border-b border-white/10 mb-6">
              <div>
                <div className="text-[10px] font-mono text-gray-400 uppercase">Target Resource</div>
            ...
            {/* State 1: Locked & 402 Challenge Simulator */}
            {simulatedState === "locked" && (
              <div className="py-8 text-center flex flex-col items-center">
                ...
                <h4 className="text-base font-bold mb-1">Protected Architecture Spec</h4>
                <p className="text-xs text-gray-400 max-w-md mb-6 leading-relaxed">
                  Full Zod rules, React 19 action patterns, and verified anti-hallucination configs are encrypted behind the x402 protocol challenge.
                </p>
                <button
                ...
                >
                  <Zap className="w-4 h-4" /> Simulate Agent x402 Micropayment (${selectedSpec.priceUSDC})
                </button>
              </div>
            )}

            {/* State 2: Challenging & Signing */}
